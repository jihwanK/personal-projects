...
tensor_node_features = torch.FloatTensor(node_features).to(device)

citation_network = dgl.graph( (df['src'], df['dst']) ).to(device)
...
def similarity_score(pair, linked_pair, features):
    ...
    if linked_pair.shape[0] == 0:
        weighted_link_similarity = torch.zeros((pair.shape[0],), dtype=torch.float32).to(device)
    else:
...
def generate_positive_negative_pairs(graph, node_features, high_threshold = 0.6, low_threshold = 0.4, n_samples=10_000):
    ...
    random_pair = torch.randint(0, n_nodes, (n_samples, 2)).to(device)
    ...
    linked_random_pair = torch.FloatTensor(linked_random_pair).to(device)
...
def train_model(model, citation_network, node_features, optimizer, criterion, epochs=100):
    loss_values = []
    ...
    for epoch in range(epochs):
        ...
        positive_pairs, negative_pairs = generate_positive_negative_pairs(citation_network, node_features, n_samples=10_000)
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        loss_contrastive = torch.mean((1-label) * torch.pow(euclidean_distance, 2) +
                                      (label) * torch.pow(torch.clamp(self.margin - euclidean_distance, min=0.0), 2)).to(device)
        return loss_contrastive
    
    
# main
model = GAT(in_dim=1200, hidden_dim=300, out_dim=100, num_heads=4).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
criterion = ContrastiveLoss()
loss_values, output = train_model(model, citation_network, tensor_node_features, optimizer, criterion, epochs=50)
torch.save(output, 'result_100_10k.pt')
...
plt.savefig('loss_trend_100_10k.png')
np.savetxt('loss_values_100_10k.csv', loss_values, delimiter=',')